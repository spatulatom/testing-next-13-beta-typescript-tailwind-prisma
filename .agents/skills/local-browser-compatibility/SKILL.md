---
name: local-browser-compatibility
...
Use this guide as a strict three-phase workflow: first measure build output, then decide whether the measured browser floor is acceptable, and only configure ongoing enforcement after the user approves Phase 3. The highest-priority rules are: resolve target browsers from prompt/config/default, include mobile browser identifiers, audit emitted build assets, record results in the findings template, and stop for user confirmation before installing tools or changing project config.

---

## Table of Contents

1. <a href="#parameters">Parameters</a>
2. <a href="#goal">Goal</a>
3. <a href="#three-dimensions-of-compatibility">Three Dimensions of Compatibility</a>
4. <a href="#phase-1-measure">Phase 1: Measure</a>

- <a href="#step-11-build-the-project">Step 1.1: Build the project</a>
- <a href="#step-12-css-features">Step 1.2: CSS features</a>
- <a href="#step-13-js-syntax">Step 1.3: JS syntax</a>
- <a href="#step-14-js-apis">Step 1.4: JS APIs</a>
- <a href="#step-15-document-findings">Step 1.5: Document findings</a>

5. <a href="#phase-2-decide">Phase 2: Decide</a>

- <a href="#step-21-review-gaps">Step 2.1: Review gaps</a>
- <a href="#step-22-check-build-tool-configuration">Step 2.2: Check build tool configuration</a>
- <a href="#step-23-discover-hardcoded-compiler-targets">Step 2.3: Discover hardcoded compiler targets</a>
- <a href="#step-24-decision-checkpoint">Step 2.4: Decision checkpoint</a>

6. <a href="#phase-3-enforce-optional">Phase 3: Enforce (Optional)</a>

- <a href="#step-30-install-tools-shared-by-3a-and-3b">Step 3.0: Install tools</a>
- <a href="#3a-development-source-code">3A: Development (Source Code)</a>
  - <a href="#3a1-css-features--stylelint">3A.1: CSS features -> Stylelint</a>
  - <a href="#3a2-js-syntax--not-needed">3A.2: JS syntax -> Not needed</a>
  - <a href="#3a3-js-apis--eslint">3A.3: JS APIs -> ESLint</a>
  - <a href="#3a4-verify-editor-integration">3A.4: Verify editor integration</a>
  - <a href="#3a5-add-development-script">3A.5: Add development script</a>
- <a href="#3b-build-output-repeatable-npm-scripts">3B: Build Output (Repeatable npm Scripts)</a>

7. <a href="#checklist">Checklist</a>

- <a href="#phase-1-measure-build-output">Phase 1: Measure</a>
- <a href="#phase-2-decide-1">Phase 2: Decide</a>
- <a href="#phase-3a-development-source-code">Phase 3A: Development</a>
- <a href="#phase-3b-build-output-repeatable-npm-scripts">Phase 3B: Build output</a>
...
>
> #### Step A: Determine priority source
>
...
> 3. **Default** — Fall back to `baseline widely available`, meaning features supported in all core browser engines for at least 30 months. Resolve the exact browser/version list at runtime with `npx browserslist "baseline widely available"` because the versions move over time.
>
> #### Step B: Document the source
>
> **Always document which source was used, regardless of which priority level applied.** If no config existed and you create `.browserslistrc`, record that it was **created** (not found).
>
> #### Step C: Include mobile browsers
>
...
>
> - `baseline widely available` — Conservative default: features supported in all core browser engines for at least 30 months; resolve exact versions with `npx browserslist "baseline widely available"`
...
> **Philosophy:** We target `baseline widely available` as our guaranteed floor: features supported in all core browser engines for at least 30 months, with exact versions resolved by Browserslist at run time. If the build supports older browsers, that's a bonus—not a goal. Chasing ancient browser support has diminishing returns.
...
Fill in this summary table from Phase 1 findings before proceeding to 2.2:

| Dimension    | Phase 1 Result                                       | Target Browser Floor | Gap?   | Proposed action            |
| ------------ | ---------------------------------------------------- | -------------------- | ------ | -------------------------- |
| CSS features | (e.g., ✅ pass / ❌ Container Queries — Chrome 105+) | (e.g., Chrome 90+)   | Yes/No | Remove / fallback / accept |
| JS syntax    | (e.g., ES2022 floor)                                 | (e.g., ES2020)       | Yes/No | Accept / lower target      |
| JS APIs      | (e.g., ✅ pass / ❌ structuredClone — Safari 15.4+)  | (e.g., Safari 14+)   | Yes/No | Polyfill / alternative     |

> **Rule:** Complete this table before moving to 2.2. It becomes the canonical reference for the decision checkpoint in 2.4.

**Detailed gap table** (expand as needed):
...
APIs are not transpiled. Your effective JS API floor is determined by which polyfills you ship. If you ship none, the floor is whatever the currently resolved `baseline widely available` browser set supports natively.
